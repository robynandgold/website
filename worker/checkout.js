...
export async function handleCheckout(request, env) {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // Shows the "Add promotion code" field on Stripe's checkout page. The
      // codes themselves are created from the admin page (/api/promos) and
      // validated by Stripe, so no discount is ever computed from anything the
      // browser sends.
      //
      // Withdrawn while a sale is running: the line items are already reduced,
      // and Stripe would apply a code on top of that. Doing it here rather
      // than in the admin page means it holds however the checkout is reached
      // — the field simply isn't offered.
      allow_promotion_codes: !saleActive(sale),
      line_items,
