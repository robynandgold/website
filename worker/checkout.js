...
export async function handleCheckout(request, env) {
  ...
  try {
    ...
    // TEST: while the checkout-test product is in the cart, offer a single
    // reduced Ireland shipping rate so the live test stays cheap. Any real
    // order (any other product) keeps the normal rates below. Remove with the
    // test product.
    const isCheckoutTest = items.some(
      (item) => item.id === 'test-do-not-buy-checkout-test'
    );
    const testShippingOptions = [
      {
        shipping_rate_data: {
          type: 'fixed_amount',
          fixed_amount: { amount: 100, currency: 'eur' },
          display_name: 'Ireland shipping (test)',
          delivery_estimate: {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      shipping_options: isCheckoutTest ? testShippingOptions : [
        {
          shipping_rate_data: {
            type: 'fixed_amount',
